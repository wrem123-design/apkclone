...
function rowsFromState(state: SNSGodState): Array<Row | GroupRow> {
  ...
  return [...directRows, ...groupRows].sort((a, b) => b.lastActivity - a.lastActivity);
}
    ...
    <View style={[styles.screen, !kakaoTheme && styles.screenDefault]}>
      <View style={[styles.header, !kakaoTheme && styles.headerDefault]}>
        ...
        <View style={styles.quickActions}>
          <Pressable accessibilityLabel="새 대화" onPress={onNewRoom} style={[styles.quickButton, !kakaoTheme && styles.quickButtonDefault]}><Text style={styles.quickIcon}>＋</Text></Pressable>
          <Pressable accessibilityLabel="새 그룹대화" onPress={onNewGroupRoom} style={[styles.quickButton, !kakaoTheme && styles.quickButtonDefault]}><Text style={styles.quickIcon}>▦</Text></Pressable>
          <Pressable accessibilityLabel="새 캐릭터" onPress={onNewCharacter} style={[styles.quickButton, !kakaoTheme && styles.quickButtonDefault]}><Text style={styles.quickIcon}>◇</Text></Pressable>
        </View>
      </View>
      <FlatList
        data={rows}
...
const styles = StyleSheet.create({
  ...
  header: { minHeight: 112, paddingHorizontal: 20, paddingTop: 16, gap: 14 },
  ...
  quickButton: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#f2f2f2', alignItems: 'center', justifyContent: 'center' },
  quickButtonDefault: { backgroundColor: '#eee8dc', borderWidth: 1, borderColor: colors.border },
  quickIcon: { color: '#333', fontWeight: '900', fontSize: 22, lineHeight: 25 },
