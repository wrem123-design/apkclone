.class public final LX/Q9i;
.super LX/04H;
.source ""


# instance fields
.field public final A00:LX/Mao;

.field public final A01:LX/2ZE;


# direct methods
.method public constructor <init>(LX/Mao;LX/2ZE;)V
    .locals 0

    invoke-static {p1}, LX/659;->A0v(Ljava/lang/Object;)V

    invoke-direct {p0}, LX/9ig;-><init>()V

    iput-object p2, p0, LX/Q9i;->A01:LX/2ZE;

    iput-object p1, p0, LX/Q9i;->A00:LX/Mao;

    return-void
.end method


# virtual methods
.method public final A0e(LX/6iO;)LX/9ig;
    .locals 39

    const/4 v6, 0x0

    move-object/from16 v5, p1

    invoke-static {v5, v6}, LX/659;->A0y(Ljava/lang/Object;I)V

    new-array v1, v6, [Ljava/lang/Object;

    const/16 v0, 0x56

    invoke-static {v5, v1, v0}, LX/C1e;->A02(LX/6iO;[Ljava/lang/Object;I)Ljava/lang/Object;

    move-result-object v23

    move-object/from16 v0, v23

    check-cast v0, Landroid/graphics/Typeface;

    move-object/from16 v23, v0

    invoke-static {}, LX/955;->A0B()Landroid/graphics/drawable/GradientDrawable;

    move-result-object v1

    const v0, 0x7f060057

    invoke-static {v5, v0}, LX/mzG;->A02(LX/mzG;I)I

    move-result v0

    invoke-virtual {v1, v0}, Landroid/graphics/drawable/GradientDrawable;->setColor(I)V

    invoke-virtual {v1, v6}, Landroid/graphics/drawable/GradientDrawable;->setShape(I)V

    iget-object v0, v5, LX/6iO;->A06:LX/2nh;

    move-object/from16 v34, v0

    iget-object v0, v0, LX/2nh;->A0B:Landroid/content/Context;

    invoke-virtual {v0}, Landroid/content/Context;->getResources()Landroid/content/res/Resources;

    move-result-object v0

    const v3, 0x7f070022

    invoke-virtual {v0, v3}, Landroid/content/res/Resources;->getDimension(I)F

    move-result v0

    invoke-virtual {v1, v0}, Landroid/graphics/drawable/GradientDrawable;->setCornerRadius(F)V

    move-object/from16 v11, p0

    iget-object v10, v11, LX/Q9i;->A01:LX/2ZE;

    iget-object v8, v10, LX/2ZE;->A09:Ljava/lang/String;

    if-nez v8, :cond_0

    const v0, 0x7f133cac

    invoke-static {v5, v0}, LX/6vO;->A0L(LX/mzG;I)Ljava/lang/String;

    move-result-object v8

    :cond_0
    iget-object v0, v10, LX/2ZE;->A0A:Ljava/lang/String;

    move-object/from16 v22, v0

    const/16 v0, 0xc

    new-instance v4, LX/lzZ;

    invoke-direct {v4, v11, v0}, LX/lzZ;-><init>(Ljava/lang/Object;I)V

    iget-boolean v9, v10, LX/2ZE;->A0G:Z

    iget-boolean v2, v10, LX/2ZE;->A0H:Z

    if-nez v9, :cond_1

    const/4 v14, 0x0

    if-eqz v2, :cond_2

    :cond_1
    const/4 v14, 0x1

    :cond_2
    iget-boolean v0, v10, LX/2ZE;->A0F:Z

    move/from16 v21, v0

    sget-object v33, LX/6wM;->A04:LX/6wM;

    sget-object v7, LX/04U;->A02:LX/6rG;

    invoke-static {v1, v7}, LX/6wB;->A01(Landroid/graphics/drawable/Drawable;LX/04U;)LX/04U;

    move-result-object v1

    invoke-static {}, LX/Asd;->A0h()LX/6W8;

    move-result-object v0

    if-ne v1, v7, :cond_3

    const/4 v1, 0x0

    :cond_3
    invoke-static {v1, v0}, LX/255;->A0q(LX/04U;LX/04V;)LX/04U;

    move-result-object v0

    invoke-static {v0, v8}, LX/6vV;->A00(LX/04U;Ljava/lang/CharSequence;)LX/04U;

    move-result-object v13

    sget-object v12, LX/6wD;->A0N:LX/6wD;

    const-string v1, "clips_pause_state_orbit_cta_pill_component"

    new-instance v0, LX/6W8;

    invoke-direct {v0, v12, v1}, LX/6W8;-><init>(LX/6wD;Ljava/lang/Object;)V

    if-ne v13, v7, :cond_4

    const/4 v13, 0x0

    :cond_4
    invoke-static {v13, v0}, LX/255;->A0q(LX/04U;LX/04V;)LX/04U;

    move-result-object v20

    const v0, 0x7f070022

    if-eqz v14, :cond_5

    const v0, 0x7f070048

    :cond_5
    invoke-static {v5, v0}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v0

    invoke-static {v5, v3}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v18

    const v12, 0x7f070048

    invoke-static {v5, v12}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v16

    invoke-static {v5, v12}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v12

    sget-object v14, LX/6vX;->A0N:LX/6vX;

    const/4 v5, 0x0

    invoke-static {v14, v0, v1}, LX/255;->A0r(LX/6vX;J)LX/6W9;

    move-result-object v15

    move-object/from16 v14, v20

    move-wide/from16 v0, v16

    invoke-static {v14, v15, v0, v1}, LX/B55;->A0b(LX/04U;LX/04V;J)LX/04U;

    move-result-object v1

    invoke-static/range {v18 .. v19}, LX/AqC;->A0W(J)LX/6W9;

    move-result-object v0

    invoke-static {v1, v0, v12, v13}, LX/AsE;->A0X(LX/04U;LX/04V;J)LX/04U;

    move-result-object v0

    invoke-static {v0, v4}, LX/6wB;->A0E(LX/04U;Lkotlin/jvm/functions/Function1;)LX/04U;

    move-result-object v4

    const/16 v1, 0xb

    new-instance v0, LX/lzZ;

    invoke-direct {v0, v11, v1}, LX/lzZ;-><init>(Ljava/lang/Object;I)V

    invoke-static {v4, v0}, LX/6xJ;->A04(LX/04U;Lkotlin/jvm/functions/Function1;)LX/04U;

    move-result-object v16

    invoke-static/range {v34 .. v34}, LX/031;->A0C(LX/2nh;)LX/04Y;

    move-result-object v4

    if-eqz v9, :cond_f

    const v0, 0x7f070016

    invoke-static {v4, v0}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v0

    sget-object v2, LX/6vX;->A06:LX/6vX;

    invoke-static {v5, v2, v0, v1}, LX/031;->A0D(LX/04U;LX/6vX;J)LX/04U;

    move-result-object v2

    iget-object v10, v4, LX/04Y;->A00:LX/2nh;

    invoke-static {v10}, LX/031;->A0C(LX/2nh;)LX/04Y;

    move-result-object v9

    invoke-static {v9, v3}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v0

    invoke-static {v5, v0, v1}, LX/Atd;->A0S(LX/04U;J)LX/04U;

    move-result-object v11

    invoke-static {v9, v3}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v0

    invoke-static {v11, v0, v1}, LX/Atd;->A0U(LX/04U;J)LX/04U;

    move-result-object v28

    const v0, 0x7f082422

    invoke-static {v9, v0}, LX/E2I;->A01(LX/ncA;I)Landroid/graphics/drawable/Drawable;

    move-result-object v26

    new-instance v0, LX/7tO;

    move-object/from16 v24, v0

    move-object/from16 v25, v5

    move-object/from16 v27, v5

    move/from16 v29, v6

    move/from16 v30, v6

    invoke-direct/range {v24 .. v30}, LX/7tO;-><init>(Landroid/content/res/ColorStateList;Landroid/graphics/drawable/Drawable;Landroid/widget/ImageView$ScaleType;LX/04U;ZZ)V

    invoke-static {v0, v9}, LX/255;->A1b(LX/9ig;LX/04Y;)Z

    move-result v0

    if-eqz v0, :cond_e

    iget-object v0, v9, LX/04Y;->A01:Ljava/util/List;

    new-instance v1, LX/Qs3;

    move-object/from16 v24, v1

    move-object/from16 v25, v2

    move-object/from16 v26, v5

    move-object/from16 v28, v5

    move-object/from16 v29, v5

    move-object/from16 v30, v5

    move-object/from16 v31, v0

    move/from16 v32, v6

    invoke-direct/range {v24 .. v32}, LX/Qs3;-><init>(LX/04U;LX/6wM;LX/6wM;LX/6wN;LX/9x5;Ljava/lang/Integer;Ljava/util/List;Z)V

    :goto_0
    invoke-virtual {v4, v1}, LX/04Y;->A00(LX/9ig;)V

    :cond_6
    iget-object v0, v4, LX/04Y;->A00:LX/2nh;

    move-object/from16 v17, v0

    invoke-static/range {v17 .. v17}, LX/031;->A0C(LX/2nh;)LX/04Y;

    move-result-object v10

    if-eqz v22, :cond_d

    invoke-virtual/range {v22 .. v22}, Ljava/lang/String;->length()I

    move-result v0

    if-eqz v0, :cond_d

    sget-object v12, LX/7MA;->A08:LX/7MA;

    :goto_1
    invoke-static {v10}, LX/6vO;->A08(LX/mzG;)J

    move-result-wide v0

    iget-object v9, v10, LX/04Y;->A00:LX/2nh;

    iget-object v11, v9, LX/2nh;->A0B:Landroid/content/Context;

    invoke-static {v11, v10}, LX/AqC;->A03(Landroid/content/Context;LX/mzG;)I

    move-result v14

    const v13, 0x7f140574

    invoke-static {}, LX/031;->A04()J

    move-result-wide v2

    invoke-static {v9, v8, v13, v14}, LX/031;->A0I(LX/2nh;Ljava/lang/CharSequence;II)LX/8vP;

    move-result-object v13

    iget-object v8, v9, LX/2nh;->A0E:LX/8jh;

    invoke-static {v13, v8, v6, v0, v1}, LX/031;->A12(LX/8vP;LX/8jh;IJ)V

    move-object/from16 v0, v23

    invoke-static {v0, v13, v8, v2, v3}, LX/031;->A0o(Landroid/graphics/Typeface;LX/8vP;LX/8jh;J)V

    invoke-static {v13, v12}, LX/031;->A10(LX/8vP;LX/7MA;)V

    const/4 v0, 0x2

    invoke-static {v13, v8, v0, v2, v3}, LX/AqD;->A1D(LX/8vP;LX/8jh;IJ)V

    const/4 v12, 0x1

    invoke-static {v7, v13, v12}, LX/BWc;->A0Q(LX/04U;LX/8vP;Z)V

    invoke-static {v10, v13}, LX/77T;->A1K(LX/04Y;LX/8vP;)V

    if-eqz v22, :cond_7

    invoke-virtual/range {v22 .. v22}, Ljava/lang/String;->length()I

    move-result v0

    if-eqz v0, :cond_7

    const v0, 0x7f070034

    invoke-static {v10, v0}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v0

    invoke-static {v5, v0, v1}, LX/Atd;->A0R(LX/04U;J)LX/04U;

    move-result-object v15

    invoke-static {v11, v10}, LX/Asd;->A03(Landroid/content/Context;LX/mzG;)I

    move-result v14

    invoke-static {v10}, LX/6vO;->A0F(LX/mzG;)J

    move-result-wide v0

    sget-object v13, Landroid/graphics/Typeface;->DEFAULT:Landroid/graphics/Typeface;

    move-object/from16 v11, v22

    invoke-static {v9, v11, v6, v14}, LX/031;->A0I(LX/2nh;Ljava/lang/CharSequence;II)LX/8vP;

    move-result-object v9

    invoke-static {v9, v8, v6, v0, v1}, LX/031;->A12(LX/8vP;LX/8jh;IJ)V

    invoke-static {v13, v9, v8, v2, v3}, LX/031;->A0o(Landroid/graphics/Typeface;LX/8vP;LX/8jh;J)V

    invoke-static {v9, v5}, LX/031;->A10(LX/8vP;LX/7MA;)V

    invoke-static {v9, v8, v12, v2, v3}, LX/BWc;->A01(LX/8vP;LX/8jh;IJ)I

    move-result v0

    invoke-static {v9, v0, v12, v6}, LX/031;->A0z(LX/8vP;IZZ)V

    invoke-static {v10, v15, v9}, LX/031;->A0y(LX/04Y;LX/04U;LX/8vP;)V

    :cond_7
    sget-boolean v0, LX/7hx;->useKFlexboxContainerComponent:Z

    if-eqz v0, :cond_c

    iget-object v1, v10, LX/04Y;->A01:Ljava/util/List;

    new-instance v0, LX/Qs0;

    move-object v8, v0

    move-object v9, v7

    move-object v10, v5

    move-object v11, v5

    move-object v12, v5

    move-object v13, v1

    move v14, v6

    invoke-direct/range {v8 .. v14}, LX/Qs0;-><init>(LX/04U;LX/6wM;LX/6wM;LX/6wN;Ljava/util/List;Z)V

    :goto_2
    invoke-virtual {v4, v0}, LX/04Y;->A00(LX/9ig;)V

    if-eqz v21, :cond_8

    if-eqz v22, :cond_b

    invoke-virtual/range {v22 .. v22}, Ljava/lang/String;->length()I

    move-result v0

    if-eqz v0, :cond_b

    invoke-static {v4}, LX/6vO;->A0A(LX/mzG;)J

    move-result-wide v0

    :goto_3
    invoke-static {v5, v0, v1}, LX/Atd;->A0T(LX/04U;J)LX/04U;

    move-result-object v2

    invoke-static/range {v17 .. v17}, LX/031;->A0C(LX/2nh;)LX/04Y;

    move-result-object v3

    invoke-static {v3}, LX/6vO;->A0A(LX/mzG;)J

    move-result-wide v0

    invoke-static {v5, v0, v1}, LX/Atd;->A0S(LX/04U;J)LX/04U;

    move-result-object v7

    invoke-static {v3}, LX/6vO;->A0A(LX/mzG;)J

    move-result-wide v0

    invoke-static {v7, v0, v1}, LX/Atd;->A0U(LX/04U;J)LX/04U;

    move-result-object v11

    if-eqz v22, :cond_a

    invoke-virtual/range {v22 .. v22}, Ljava/lang/String;->length()I

    move-result v0

    if-eqz v0, :cond_a

    const v0, 0x7f082150

    invoke-static {v3, v0}, LX/AqC;->A0B(LX/mzG;I)Landroid/graphics/drawable/Drawable;

    move-result-object v1

    iget-object v0, v3, LX/04Y;->A00:LX/2nh;

    iget-object v0, v0, LX/2nh;->A0B:Landroid/content/Context;

    invoke-static {v0}, LX/06B;->A0P(Landroid/content/Context;)I

    move-result v0

    invoke-static {v1, v3, v0}, LX/Atd;->A1A(Landroid/graphics/drawable/Drawable;LX/mzG;I)V

    :goto_4
    invoke-static {v1}, LX/659;->A0w(Ljava/lang/Object;)V

    new-instance v0, LX/7tO;

    move-object v7, v0

    move-object v8, v5

    move-object v9, v1

    move-object v10, v5

    move v12, v6

    move v13, v6

    invoke-direct/range {v7 .. v13}, LX/7tO;-><init>(Landroid/content/res/ColorStateList;Landroid/graphics/drawable/Drawable;Landroid/widget/ImageView$ScaleType;LX/04U;ZZ)V

    invoke-static {v0, v3}, LX/255;->A1b(LX/9ig;LX/04Y;)Z

    move-result v0

    if-eqz v0, :cond_9

    iget-object v1, v3, LX/04Y;->A01:Ljava/util/List;

    new-instance v0, LX/Qs3;

    move-object v7, v0

    move-object v8, v2

    move-object v9, v5

    move-object v11, v5

    move-object v12, v5

    move-object v13, v5

    move-object v14, v1

    move v15, v6

    invoke-direct/range {v7 .. v15}, LX/Qs3;-><init>(LX/04U;LX/6wM;LX/6wM;LX/6wN;LX/9x5;Ljava/lang/Integer;Ljava/util/List;Z)V

    :goto_5
    invoke-virtual {v4, v0}, LX/04Y;->A00(LX/9ig;)V

    :cond_8
    sget-boolean v0, LX/7hx;->useKFlexboxContainerComponent:Z

    if-eqz v0, :cond_11

    iget-object v0, v4, LX/04Y;->A01:Ljava/util/List;

    new-instance v30, LX/Qs3;

    move-object/from16 v31, v16

    move-object/from16 v32, v5

    move-object/from16 v34, v5

    move-object/from16 v35, v5

    move-object/from16 v36, v5

    move-object/from16 v37, v0

    move/from16 v38, v6

    invoke-direct/range {v30 .. v38}, LX/Qs3;-><init>(LX/04U;LX/6wM;LX/6wM;LX/6wN;LX/9x5;Ljava/lang/Integer;Ljava/util/List;Z)V

    return-object v30

    :cond_9
    move-object/from16 v0, v17

    invoke-static {v0, v3, v2}, LX/6rL;->A0G(LX/2nh;LX/04Y;LX/04U;)LX/8ch;

    move-result-object v0

    goto :goto_5

    :cond_a
    const v0, 0x7f082150

    invoke-static {v3, v0}, LX/E2I;->A01(LX/ncA;I)Landroid/graphics/drawable/Drawable;

    move-result-object v1

    goto :goto_4

    :cond_b
    invoke-static {v4}, LX/6vO;->A0H(LX/mzG;)J

    move-result-wide v0

    goto/16 :goto_3

    :cond_c
    move-object/from16 v0, v17

    invoke-static {v0, v10, v7}, LX/6rL;->A03(LX/2nh;LX/04Y;LX/04U;)LX/8cc;

    move-result-object v0

    goto/16 :goto_2

    :cond_d
    sget-object v12, LX/7MA;->A02:LX/7MA;

    goto/16 :goto_1

    :cond_e
    invoke-static {v10, v9, v2}, LX/6rL;->A0G(LX/2nh;LX/04Y;LX/04U;)LX/8ch;

    move-result-object v1

    goto/16 :goto_0

    :cond_f
    if-eqz v2, :cond_6

    iget-object v2, v10, LX/2ZE;->A08:Ljava/lang/String;

    if-eqz v2, :cond_6

    const v0, 0x7f070016

    invoke-static {v4, v0}, LX/6vO;->A0J(LX/mzG;I)J

    move-result-wide v0

    sget-object v3, LX/6vX;->A06:LX/6vX;

    invoke-static {v5, v3, v0, v1}, LX/031;->A0D(LX/04U;LX/6vX;J)LX/04U;

    move-result-object v9

    iget-object v10, v4, LX/04Y;->A00:LX/2nh;

    invoke-static {v10}, LX/031;->A0C(LX/2nh;)LX/04Y;

    move-result-object v3

    iget-object v12, v3, LX/04Y;->A00:LX/2nh;

    invoke-static {v12}, LX/QqB;->A07(LX/2nh;)LX/P8f;

    move-result-object v11

    const-string v1, "orbit_cta_pill_image"

    iget-object v0, v11, LX/P8f;->A00:LX/QqB;

    iput-object v1, v0, LX/QqB;->A06:Ljava/lang/Object;

    const/4 v1, 0x0

    :try_start_0
    invoke-static {v2}, LX/BRW;->A04(Ljava/lang/String;)Landroid/net/Uri;

    move-result-object v1
    :try_end_0
    .catch Ljava/lang/SecurityException; {:try_start_0 .. :try_end_0} :catch_0
    .catch Ljava/lang/UnsupportedOperationException; {:try_start_0 .. :try_end_0} :catch_0

    :catch_0
    iget-object v0, v11, LX/P8f;->A00:LX/QqB;

    iput-object v1, v0, LX/QqB;->A01:Landroid/net/Uri;

    iget-object v0, v12, LX/2nh;->A0B:Landroid/content/Context;

    invoke-virtual {v0}, Landroid/content/Context;->getResources()Landroid/content/res/Resources;

    move-result-object v1

    const v0, 0x7f070021

    invoke-virtual {v1, v0}, Landroid/content/res/Resources;->getDimension(I)F

    move-result v2

    invoke-static {}, LX/955;->A0M()LX/4ch;

    move-result-object v1

    sget-object v0, LX/4ck;->A01:LX/4ck;

    invoke-virtual {v1, v0}, LX/4ch;->A02(LX/4ck;)V

    invoke-static {v2}, LX/Yty;->A00(F)LX/0ZN;

    move-result-object v0

    iput-object v0, v1, LX/4ch;->A0L:LX/0ZN;

    invoke-static {v11, v1}, LX/BWc;->A0H(LX/P8f;LX/4ch;)V

    const v2, 0x7f07001e

    iget-object v1, v11, LX/BWc;->A02:LX/8jh;

    invoke-virtual {v1, v2}, LX/8jh;->A02(I)I

    move-result v0

    invoke-virtual {v11, v0}, LX/BWc;->A0y(I)V

    invoke-virtual {v1, v2}, LX/8jh;->A02(I)I

    move-result v0

    invoke-virtual {v11, v0}, LX/BWc;->A0x(I)V

    invoke-static {v11, v3}, LX/BWc;->A0I(LX/P8f;LX/04Y;)V

    sget-boolean v0, LX/7hx;->useKFlexboxContainerComponent:Z

    if-eqz v0, :cond_10

    iget-object v0, v3, LX/04Y;->A01:Ljava/util/List;

    new-instance v1, LX/Qs3;

    move-object/from16 v24, v1

    move-object/from16 v25, v9

    move-object/from16 v26, v5

    move-object/from16 v27, v5

    move-object/from16 v28, v5

    move-object/from16 v29, v5

    move-object/from16 v30, v5

    move-object/from16 v31, v0

    move/from16 v32, v6

    invoke-direct/range {v24 .. v32}, LX/Qs3;-><init>(LX/04U;LX/6wM;LX/6wM;LX/6wN;LX/9x5;Ljava/lang/Integer;Ljava/util/List;Z)V

    goto/16 :goto_0

    :cond_10
    invoke-static {v10, v3, v9}, LX/6rL;->A0G(LX/2nh;LX/04Y;LX/04U;)LX/8ch;

    move-result-object v1

    goto/16 :goto_0

    :cond_11
    move-object/from16 v1, v34

    move-object/from16 v0, v16

    invoke-static {v1, v4, v0}, LX/6rL;->A0E(LX/2nh;LX/04Y;LX/04U;)LX/8ch;

    move-result-object v30

    return-object v30
.end method
